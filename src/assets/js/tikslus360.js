...
(function ($) {
    var Tikslus360 = function (element, options) {
        ...
        var init_ = function () {
            ...
            view360.css({ width: options.canvasWidth + "px", height: options.canvasHeight + "px", position: 'absolute' });
            ...
            //     zoomOn = 0;
            //     $(this).css({ cursor: 'e-resize' });
            //     if (options.autoRotate == false) { start_auto_rotate(); }
            // });

            $("#videoDemoId").mousedown(function (e) {
                bMouseDown = false;
                zoomOn = 0;
                $(this).css({ cursor: 'e-resize' });
                if (options.autoRotate == true) { stop_auto_rotate(); }
            ...
            $("#videoDemoId").mousemove(function (e) {
                if (options.autoRotate == false && bMouseDown == false) {
                    var wid = $("#videoDemoId").width();
                    var canvasOffset = $("#videoDemoId").offset();
                    iMouseX = Math.floor(e.pageX - canvasOffset.left);
                    iMouseY = Math.floor(e.pageY - canvasOffset.top);
                    modulus = Math.ceil(iMouseX / (wid / options.imageCount));
                    if (modulus <= 0) { modulus = 1 } else if (modulus > options.imageCount) { modulus = options.imageCount } else { };
                    rotate360(modulus);
                }
            });

            $("#videoDemoId").mouseup(function (e) { 
                bMouseDown = true;
                zoomOn = 0;
                $(this).css({ cursor: 'e-resize' });
                if (options.autoRotate == false) { start_auto_rotate(); }
